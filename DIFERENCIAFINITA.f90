...
REAL, DIMENSION(:), ALLOCATABLE     :: datos, seno, coseno
...
DO orden=1,2
	DO  i=1,n-1
		...
		V(orden,i)=ADELANTADA(seno(i),seno(i+1),seno(i+2),h,orden)
	END DO

	DO i=3,n+1
		V(orden+2,i)=ATRASADO(seno(i),seno(i-1),seno(i-2),h,orden)
	END DO

END DO

DO i=2,n
		V(5,i)=CENTRADO(seno(i),seno(i+1),0.,seno(i-1),0.,h,2)
END DO

DO i=3,n-1
		V(6,i)=CENTRADO(seno(i),seno(i+1),seno(i+2),seno(i-1),seno(i-2),h,4)
END DO
